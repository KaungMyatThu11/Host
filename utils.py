...
async def send_media(
    ...
    async def _send_once(cap, ents):
        ...
        if media_type == "audio":
            ...
            return
        if media_type == "voice":
            duration, _, _, _, _ = await get_media_info(media_path)
        ...
        if media_type == "video_note":
            duration, _, _, _, _ = await get_media_info(media_path)
            thumb = await get_video_thumbnail(media_path, duration or 0)
            sent_message = await message.reply_video_note(
                media_path,
                duration=duration or 0,
                length=360,
                thumb=thumb,
    ...
    for attempt in range(2):
        try:
            ...
            if "ENTITY_TEXT_INVALID" in str(e) and attempt == 0 and media_type not in {"sticker", "video_note"}:
